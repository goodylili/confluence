...
fun create_campaign_sui_happy_path() {
    ...
    let (c, admin) = campaign::create_campaign<SUI>(title, desc, goal, duration, &clock, test_scenario::ctx(&mut scenario));
    ...
    transfer::public_transfer<campaign::AdminCap>(admin, creator);
...
fun contribute_and_goal_reach_marks_success() {
    ...
    let (mut c, admin) = campaign::create_campaign<SUI>(string::utf8(b"Test"), string::utf8(b"Desc"), 100u64, 5_000u64, &clock, test_scenario::ctx(&mut scenario));
...
fun pause_and_unpause_flow() {
    ...
    // only admin (cap) can pause/unpause
    campaign::pause_campaign<SUI>(&mut c, &admin, string::utf8(b"maintenance"), &clock, test_scenario::ctx(&mut scenario));
    assert!(campaign::is_paused(&c), 20);

    campaign::unpause_campaign<SUI>(&mut c, &admin, &clock, test_scenario::ctx(&mut scenario));
...
fun finalize_success_then_withdraw() {
    ...
    campaign::finalize_campaign<SUI>(&mut c, &admin, &clock, test_scenario::ctx(&mut scenario));
    assert!(campaign::is_successful(&c), 31);

    // withdraw funds by admin
    campaign::withdraw_funds<SUI>(&mut c, &admin, 100, &clock, test_scenario::ctx(&mut scenario));
...
fun create_campaign_empty_title_fails() {
    ...
    let (_c, _admin) = campaign::create_campaign<SUI>(title, desc, 10, 2_000, &clock, test_scenario::ctx(&mut scenario));
    campaign::share_for_testing(_c);
    transfer::public_transfer<campaign::AdminCap>(_admin, creator);
    clock::destroy_for_testing(clock);
    test_scenario::end(scenario);
}
...
fun withdraw_before_end_fails() {
    ...
    campaign::withdraw_funds<SUI>(&mut c, &admin, 1, &clock, test_scenario::ctx(&mut scenario));
    transfer::public_transfer<campaign::AdminCap>(admin, creator);
    campaign::share_for_testing(c);
    clock::destroy_for_testing(clock);
    test_scenario::end(scenario);
